...
class sources:
    ...
    def sourcesProcessTorrents(self, torrent_sources):#adjusted Poison code
    ...
    def getConstants(self):
        self.itemProperty = 'plugin.video.poison.container.items'

        self.metaProperty = 'plugin.video.poison.container.meta'

        self.sourceFile = control.providercacheFile

        if control.condVisibility('System.HasAddon(script.module.poisonscrapers)'):
            scraperSetting = control.setting('module.provider')
        else:
            scraperSetting = control.setting('module.provider.alt')

        oas_module_name = 'PoisonScrapers (' + str(control.addon('script.module.poisonscrapers').getSetting('package.folder')) + ' set):' \
                          if control.addon('script.module.poisonscrapers').getSetting('package.folder') != 'PoisonScrapers' else 'PoisonScrapers:'

        import poisonscrapers
        sourceDir1 = poisonscrapers.sources()
        from resources.lib import sources
        sourceDir2 = sources.sources()
        try:
            import lockdown
            sourceDir3 = lockdown.sources()
        except:
            pass

        try:
            if scraperSetting == 'PoisonScrapers':
            ...
            elif scraperSetting == 'PoisonScrapers':
                self.sourceDict = sourceDir3
                self.module_name = 'lockdown:'
            #elif scraperSetting == 'Built-in':
                #self.sourceDict = sourceDir2
                #self.module_name = 'Built-in providers:'
            elif scraperSetting == 'PoisonScrapers + Built-in':
                self.sourceDict = sourceDir1 + sourceDir2
                self.module_name = 'Built-in + ' + oas_module_name
            elif scraperSetting == 'lockdown + Built-in':
                self.sourceDict = sourceDir3 + sourceDir2
                self.module_name = 'Built-in + lockdown:'
            else:
                self.sourceDict = sourceDir1
                self.module_name = oas_module_name
                control.setSetting('module.provider', 'PoisonScrapers')
        except:
            return
