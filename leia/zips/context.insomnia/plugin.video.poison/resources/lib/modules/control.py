...
def artPath():
    theme = appearance()
    return os.path.join(xbmcaddon.Addon('plugin.video.poison').getAddonInfo('path'), 'resources', 'artwork')


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.poison.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.poison.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.poison.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.poison.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
...
def clean_settings():#Poison code
    ...
    def _make_content(dict_object):
        ...
        return content
    try:
        for addon_id in ('plugin.video.poison', 'script.module.poisonscrapers'):
            removed_settings = []
            active_settings = []
