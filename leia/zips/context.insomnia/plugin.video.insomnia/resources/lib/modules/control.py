...
def setting(id, fallback=None):
	try: settings_dict = jsloads(homeWindow.getProperty('insomnia_settings'))
...
def make_settings_dict(): # service runs upon a setting change
	try:
		...
		homeWindow.setProperty('insomnia_settings', jsdumps(settings_dict))
		refresh_playAction()
		refresh_libPath()
...
def getINSOMNIAVersion():
	return xbmcaddon.Addon('plugin.video.insomnia').getAddonInfo('version')
...
def artPath():
	theme = appearance()
	return joinPath(xbmcaddon.Addon('plugin.video.insomnia').getAddonInfo('path'), 'resources', 'artwork', theme)
...
def getColor(n):
	colorChart = ['blue', 'red', 'yellow', 'deeppink', 'cyan', 'lawngreen', 'gold', 'magenta', 'orange',
...
def trigger_widget_refresh(): # should prob make this run only on "isINSOMNIA_widget"
	import time
...
def refresh_playAction(): # for insomnia global CM play actions
	play_mode = setting('play.mode')
	autoPlay = 'true' if play_mode == '1' else ''
	homeWindow.setProperty('insomnia.autoPlay.enabled', autoPlay)

def refresh_libPath(): # for insomnia global CM library actions
	homeWindow.setProperty('insomnia.movieLib.path', transPath(setting('library.movie')))
	homeWindow.setProperty('insomnia.tvLib.path', transPath(setting('library.tv')))
