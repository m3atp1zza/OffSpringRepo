...
from lescrapers import sources as fs_sources
...
class Sources:
	...
	def play(self, title, year, imdb, tmdb, tvdb, season, episode, tvshowtitle, premiered, meta, select, rescrape=None):
		gdriveEnabled = control.addon('script.module.lescrapers').getSetting('gdrive.cloudflare_url') != ''
...
## - compare meta received to database and use largest(eventually switch to a request to fetch missing db meta for item)
			...
			if not self.tmdb_key: self.tmdb_key = 'f0bbe0dc0bd0742a6ff38a7d6106f128'
...
##################
		...
		try:
			...
			if action == 'play_Item' and self.uncached_chosen != True:
				return self.playItem(title, items, chosen_source.getProperty('insomnia.source_dict'), self.meta)
			else:
				control.cancelPlayback()
		except:
			log_utils.error('Error sourceSelect(): ')
	...
	def getConstants(self):
		# self.itemProperty = 'plugin.video.insomnia.container.items'
		self.metaProperty = 'plugin.video.insomnia.container.meta'
		self.seasonProperty = 'plugin.video.insomnia.container.season'
		self.episodeProperty = 'plugin.video.insomnia.container.episode'
		self.titleProperty = 'plugin.video.insomnia.container.title'
		self.imdbProperty = 'plugin.video.insomnia.container.imdb'
		self.tmdbProperty = 'plugin.video.insomnia.container.tmdb'
		self.tvdbProperty = 'plugin.video.insomnia.container.tvdb'
		self.labelProperty = 'plugin.video.insomnia.container.label'
