...
class indexer:
    ...
    def get(self, url):
            self.list = self.insomnia_list(url)
            self.worker()
    ...
    def getx(self, url, worker=False):
            ...
            url = regex.resolve(r)
            self.list = self.insomnia_list('', result=url)
            self.addDirectory(self.list)
            return self.list
    ...
    def developer(self):
            ...
            result = f.read()
            f.close()
            self.list = self.insomnia_list('', result=result)
            for i in self.list:
    ...
    def insomnia_list(self, url, result=None):
            ...
            for item in items:
                ...
                if item.startswith('<item>'): action = 'insomnialist_play'
                elif item.startswith('<plugin>'): action = 'plugin'
                elif item.startswith('<info>') or url == '0': action = '0'
                else: action = 'directory'
                if action == 'insomnialist_play' and reglist: action = 'xdirectory'
