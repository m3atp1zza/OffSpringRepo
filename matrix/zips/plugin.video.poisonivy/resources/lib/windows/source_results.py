...
class SourceResultsXML(BaseDialog):
			...
			if action_id in self.selection_actions:
				...
				if 'UNCACHED' in source:
					...
					self.execute_code('RunPlugin(plugin://plugin.video.poisonivy/?action=cacheTorrent&caller=%s&type=%s&title=%s&items=%s&url=%s&source=%s&meta=%s)' %
											(debrid, link_type, sysname, quote_plus(jsdumps(self.results)), quote_plus(chosen_source.getProperty('poisonivy.url')), quote_plus(source_dict), quote_plus(jsdumps(self.meta))))
					self.selected = (None, '')
				else:
				...
				if cm_action == 'sourceInfo':
					...
					debrid = chosen_source.getProperty('poisonivy.debrid')
					name = chosen_source.getProperty('poisonivy.name')
					hash = chosen_source.getProperty('poisonivy.hash')
					self.execute_code('RunPlugin(plugin://plugin.video.poisonivy/?action=showDebridPack&caller=%s&name=%s&url=%s&source=%s)' %
									(quote_plus(debrid), quote_plus(name), quote_plus(chosen_source.getProperty('poisonivy.url')), quote_plus(hash)))
					self.selected = (None, '')
					...
					self.execute_code('RunPlugin(plugin://plugin.video.poisonivy/?action=download&name=%s&image=%s&source=%s&caller=sources&title=%s)' %
	...
	def set_properties(self):
		try:
			...
			# self.setProperty('poisonivy.title', title)
			...
			self.setProperty('poisonivy.premiered', new_date)
			...
			self.setProperty('poisonivy.highlight.color', getSourceHighlightColor())
		except:
			from resources.lib.modules import log_utils
			log_utils.error()
