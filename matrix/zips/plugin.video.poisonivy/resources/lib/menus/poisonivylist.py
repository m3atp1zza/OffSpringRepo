...
class indexer:
    ...
    def get(self, url):
            self.list = self.poisonivy_list(url)
            self.worker()
    ...
    def getx(self, url, worker=False):
            ...
            url = regex.resolve(r)
            self.list = self.poisonivy_list('', result=url)
            self.addDirectory(self.list)
            return self.list
    ...
    def developer(self):
            ...
            result = f.read()
            f.close()
            self.list = self.poisonivy_list('', result=result)
            for i in self.list:
    ...
    def poisonivy_list(self, url, result=None):
            ...
            for item in items:
                ...
                if item.startswith('<item>'): action = 'poisonivylist_play'
                elif item.startswith('<plugin>'): action = 'plugin'
                elif item.startswith('<info>') or url == '0': action = '0'
                else: action = 'directory'
                if action == 'poisonivylist_play' and reglist: action = 'xdirectory'
    ...
    def root_lista(self):
        try:
            regex.clear()
            url = 'https://raw.githubusercontent.com/techecoyote/LooNaticsAsylum/master/text/oneclick/oneclick.xml'
            self.list = self.poisonivy_list(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
