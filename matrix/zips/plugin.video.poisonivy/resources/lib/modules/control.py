...
def setting(id, fallback=None):
	try: settings_dict = jsloads(homeWindow.getProperty('poisonivy_settings'))
...
def make_settings_dict(): # service runs upon a setting change
	try:
		...
		homeWindow.setProperty('poisonivy_settings', jsdumps(settings_dict))
		refresh_playAction()
		refresh_libPath()
...
def getPoisonIvyVersion():
	return xbmcaddon.Addon('plugin.video.poisonivy').getAddonInfo('version')
...
def artPath():
	theme = appearance()
	return joinPath(xbmcaddon.Addon('plugin.video.poisonivy').getAddonInfo('path'), 'resources', 'artwork', theme)
...
def trigger_widget_refresh(): # should prob make this run only on "isPoisonIvy_widget"
	import time
...
def refresh_playAction(): # for poisonivy global CM play actions
	play_mode = setting('play.mode')
	autoPlay = 'true' if play_mode == '1' else ''
	homeWindow.setProperty('poisonivy.autoPlay.enabled', autoPlay)

def refresh_libPath(): # for poisonivy global CM library actions
	homeWindow.setProperty('poisonivy.movieLib.path', transPath(setting('library.movie')))
	homeWindow.setProperty('poisonivy.tvLib.path', transPath(setting('library.tv')))
