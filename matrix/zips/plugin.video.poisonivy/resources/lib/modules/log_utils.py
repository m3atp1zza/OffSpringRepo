...
LOGDEBUG = xbmc.LOGDEBUG #0  only prints when poisonivy logging set to "Debug" (LOGINFO ads to list if Leia(18))
# ###--from here down methods print when PoisonIvy logging set to "Normal".
LOGINFO = xbmc.LOGINFO #1 (doesn't print unless PoisonIvy logging set to "Debug" in Leia(18))
...
DEBUGPREFIX = '[COLOR yellowgreen][ PoisonIvy: %s ][/COLOR]'
...
def log(msg, caller=None, level=LOGNOTICE):
	...
	try:
		...
		if debug_location == '1':
			log_file = joinPath(LOGPATH, 'poisonivy.log')
		...
		import traceback
		traceback.print_exc()
		xbmc.log('[ plugin.video.poisonivy ] log_utils.log() Logging Failure: %s' % (e), LOGERROR)
...
def error(message=None, exception=True):
	try:
		...
		xbmc.log('[ plugin.video.poisonivy ] log_utils.error() Logging Failure: %s' % (e), LOGERROR)
